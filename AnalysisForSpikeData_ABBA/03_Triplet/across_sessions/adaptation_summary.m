...
BrainArea = 'All'; % either 'Core', 'Belt', or 'All'

% load file...
domo = load(fullfile(file_dir,strcat('Adaptation_Domo_',BrainArea)));
cassius = load(fullfile(file_dir,strcat('Adaptation_Cassius_',BrainArea)));
...
dPrime_A_stdiff = permute(nanmean(dPrime_A(1:3,:,:,:),1),[2 3 4 1]);
dPrime_B1_stdiff = permute(nanmean(dPrime_B1(1:3,:,:,:),1),[2 3 4 1]);
dPrime_B2_stdiff = permute(nanmean(dPrime_B2(1:3,:,:,:),1),[2 3 4 1]);

% count number of units to be avaraged...
nA = sum(~isnan(dPrime_A(1:3,:,:,:)),4);
nB1 = sum(~isnan(dPrime_B1(1:3,:,:,:)),4);
nB2 = sum(~isnan(dPrime_B2(1:3,:,:,:)),4);
% take average of adapting triplets
dpa_mean = nanmean(dPrime_A(1:3,:,:,:),4);
dpb1_mean = nanmean(dPrime_B1(1:3,:,:,:),4);
dpb2_mean = nanmean(dPrime_B2(1:3,:,:,:),4);
dpa_sem = nanstd(dPrime_A(1:3,:,:,:),0,4) ./ sqrt(nA);
dpb1_sem = nanstd(dPrime_B1(1:3,:,:,:),0,4) ./ sqrt(nB1);
dpb2_sem = nanstd(dPrime_B2(1:3,:,:,:),0,4) ./ sqrt(nB2);
clear nA nB1 nB2
...
shift = ones(3,1) * [-0.15 -0.10 -0.05 0 0.05 0.10 0.15];
x = (1:3)' * ones(1,length(list_st_all));
X = x + shift;
% plot data
figure('Position',[300, 500, 700, 500]);
subplot(2,3,1); % A hit trials
