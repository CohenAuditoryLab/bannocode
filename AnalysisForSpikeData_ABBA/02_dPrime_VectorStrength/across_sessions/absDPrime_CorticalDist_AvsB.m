...
%     a1.active_layer = [a1.active_layer active_layer];
    ...
    a1.dprime = [a1.dprime; abs(AvsB.dprime(:))]; % absolute value
    a1.dprime_all = cat(2,a1.dprime_all,abs(AvsB.dprime_all)); % absolute value
    a1.stdiff = [a1.stdiff; AvsB.stdiff(:)]; % semitone difference
    a1.group = [a1.group; AvsB.group(:)]; % hit, miss, fa
%     meanDP_a1 = [meanDP_a1 nanmean(abs(AvsB.dprime_all),2)]; % absolute value
    
    % redefine meanDP
    dp_hit = AvsB.dprime(:,1,:);
    stdiff_hit = AvsB.stdiff(:,1,:);
...
%         a1.dprime_ch{i} = [a1.dprime_ch{i} abs(AvsB.dprime_all(:,clInfo.active_channel==i-1))]; % absolute value
%     end
%     for j=1:5
%         a1.dprime_layer{j} = [a1.dprime_layer{j} abs(AvsB.dprime_all(:,active_layer==j))]; % absolute value
%     end
%     clear active_layer
end
...
%     belt.active_layer = [belt.active_layer active_layer];
    ...
    belt.dprime = [belt.dprime; abs(AvsB.dprime(:))]; % absolute value
    belt.dprime_all = cat(2,belt.dprime_all,abs(AvsB.dprime_all)); % absolute value
    belt.stdiff = [belt.stdiff; AvsB.stdiff(:)]; % semitone difference
    belt.group = [belt.group; AvsB.group(:)]; % hit, miss, fa
%     meanDP_belt = [meanDP_belt nanmean(abs(AvsB.dprime_all),2)]; % absolute value
...
%         belt.dprime_ch{i} = [belt.dprime_ch{i} abs(AvsB.dprime_all(:,clInfo.active_channel==i-1))]; % absolute value
%     end
%     for j=1:5
%         belt.dprime_layer{j} = [belt.dprime_layer{j} abs(AvsB.dprime_all(:,active_layer==j))]; % absolute value
%     end
%     clear active_layer
end
