...
for ff=1:length(LIST_DATE_A1)
    ...
    a1.stdiff = [a1.stdiff; Adapt.stdiff(:)];
    a1.group = [a1.group; Adapt.group(:)];
...
for ff=1:length(LIST_DATE_BELT)
    ...
    belt.stdiff = [belt.stdiff; Adapt.stdiff(:)];
    belt.group = [belt.group; Adapt.group(:)];
...
set(gca,'xTickLabel',{'supra','granular','infra'},'ylim',[0 2.5]);
x_lim = get(gca,'xlim');
% xlabel('layer');
