function [] = checkAuditoryResponse_phy2_Domo( BLOCK_NAME )
% 3/11/21 added Kruskal-Wallis test for the statistically significant response

% clear all;
% BLOCK_NAME = '20191009_ABBA_d03';
...
NDATA_DIR = fullfile('/Volumes/TOSHIBA_EXT/Kilosort/Domo', BLOCK_NAME);
...
for n=1:numel(list_cluster)
    ...
    if sum(h_temp)==0 % no significance...
        ...
        resp = mean(c_peristim(:,2:4),1) - mean(c_peristim(:,1),1);
        resp(h_w==0) = NaN;
        if sum(resp<0) == sum(~isnan(resp)) % suppressive response 
            if find(resp == min(resp),1) == 1
                h_AorB(n) = 1; % respond to tone A
            else
                h_AorB(n) = 2; % respond to tone B
            end
        else % positive response for at least one or the ABB triplet 
            if find(resp == max(resp),1) == 1
                h_AorB(n) = 1; % respond to tone A
            else
                h_AorB(n) = 2; % respond to tone B
            end
    ...
    clear h_temp pp p_w h_w p_kw h_kw resp
