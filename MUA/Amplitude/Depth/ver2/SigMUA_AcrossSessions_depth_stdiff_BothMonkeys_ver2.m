% separate data into supragranular and infragranular layer (extreme of
% superficial and deep layers excluding granular layer)
...
for ff=1:numel(list_RecDate)
    ...
    if strcmp(layer,'Sup')
%         L_ch = L3u_channel(ff); % get L3 upper
        L_ch = L3_channel(ff); % get L3 lower
        [~,~,resp,depth] = get_MUASignificance_tpos_stdiff(rec_date,L_ch,tpos,0); % do not save figure
        i_depth = 1:length(depth);
%         i_depth = i_depth(depth>0); % select superficial layer
        i_depth = i_depth(depth>=0); % select superficial layer (excluding L_ch itself)
    elseif strcmp(layer,'Deep')
        L_ch = L5_channel(ff); % get L5 
        [~,~,resp,depth] = get_MUASignificance_tpos_stdiff(rec_date,L_ch,tpos,0); % do not save figure
        i_depth = 1:length(depth);
    ...
    if isnan(L_ch) % layers cannot be determined in some sessions...
...
%     root_save_file_dir = 'E:\01_Research\01_STREAM_INTEGRATION&SEGREGATION\ANALYSIS\MUA\Results\AcrossSessions\Response';
    root_save_file_dir = fullfile(root_dir,'AcrossSessions','Response','Depth_ver2');
