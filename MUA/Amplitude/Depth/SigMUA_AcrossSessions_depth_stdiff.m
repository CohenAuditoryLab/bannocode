...
%     [sig,sigResp,resp,depth] = get_MUASignificance_tpos(rec_date,L3_ch,tpos,isSave);
    ...
    s(i_depth,:) = NaN; % select depth
    ss = cat(3,s,s);
    SIG = cat(4,SIG,ss);
    clear s ss
    
    p = repmat(pref.A,[1 2]);
    p(i_depth,:) = NaN; % select depth
    pp = cat(3,p,p);
    PREF = cat(4,PREF,pp);
    clear p pp
end
...
%     root_save_file_dir = '/Volumes/TOSHIBA_EXT/01_STREAMING/MUA/Results/AcrossSessions/Response';
    root_save_file_dir = 'E:\01_Research\01_STREAM_INTEGRATION&SEGREGATION\ANALYSIS\MUA\Results\AcrossSessions\Response\Depth';
