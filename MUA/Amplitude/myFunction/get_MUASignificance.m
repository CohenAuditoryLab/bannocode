function [ sig, sigResp, resp, chDepth ] = get_MUASignificance( RecordingDate, L3_ch, isSave )
...
% for 6 triplet repetition (original)
% resp_A  = squeeze(mean(zMUA_A,1)); % stdiff x HvsM x channel
% resp_B1 = squeeze(mean(zMUA_B1,1));
% resp_B2 = squeeze(mean(zMUA_B2,1));

% only for first 3 triplets...
resp_A  = squeeze(mean(zMUA_A(1:3,:,:,:),1)); % stdiff x HvsM x channel
resp_B1 = squeeze(mean(zMUA_B1(1:3,:,:,:),1));
resp_B2 = squeeze(mean(zMUA_B2(1:3,:,:,:),1));
...
if isSave==1
    ...
    save_file_name = strcat(RecordingDate,'_MUASignificance');
