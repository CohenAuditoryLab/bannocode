function [ sig, sigResp, resp, chDepth ] = get_MUASignificance_tpos( RecordingDate, L3_ch, tpos, isSave )
...
% tpos = 1; % triplet position 1, 2 or 3
...
resp_A  = squeeze(mean(zMUA_A(tpos,:,:,:),1)); % stdiff x HvsM x channel
resp_B1 = squeeze(mean(zMUA_B1(tpos,:,:,:),1));
resp_B2 = squeeze(mean(zMUA_B2(tpos,:,:,:),1));
...
if isSave==1
    ...
    save_file_name = strcat(RecordingDate,'_MUASignificance_',s_triplet(tpos),'Triplet');
