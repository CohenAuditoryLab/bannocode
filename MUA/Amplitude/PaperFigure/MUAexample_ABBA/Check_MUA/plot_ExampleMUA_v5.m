function [] = plot_ExampleMUA_v5(ANIMAL,RecordingDate,ch,isLegend)
...
chMUA_onset = squeeze(MUA_onset(:,ch,:,:)); % time x dF x hit-miss
chMUA_target = squeeze(MUA_target(:,ch,:,:)); % time x dF x hit-miss
...
h(2) = subplot(3,5,4:5);
l = plot(t_target,squeeze(chMUA_target(:,1,:)),'LineWidth',1.5); % plot small dF trial
...
if strcmp(isLegend,'y')
    legend({'hit','miss'},'Location',[0.18 0.85 0.1 0.1],'AutoUpdate','off');
end

% plot MUA (large dF trial)
% MUA around stimulus onset
h(3) = subplot(3,5,6:8);
l = plot(t_onset,squeeze(chMUA_onset(:,2,:)),'LineWidth',1.5); % plot large dF trial
set(gca,'XLim',w_onset); % first triplet (with 25 ms delay);
set(gca,'XTick',0:225:w_onset(2));
y_range(3,:) = get(gca,'YLim');
% xlabel('Time from Stimulus Onset [ms]'); 
ylabel('Amplitude [\muV]');
title('large \DeltaF trial');
box off;
for k=1:2
    set(l(k),'Color',lColor{k});
end
...
l = plot(t_target,squeeze(chMUA_target(:,2,:)),'LineWidth',1.5); % plot large dF trial
set(gca,'XLim',w_target);
set(gca,'XTick',-225:225:w_target(2));
...
% plot(t_onset,diff(chMUA_onset(:,:,1),1,2),'LineWidth',1.5); % ldF - sdF
plot(t_onset,-diff(chMUA_onset,1,3),'LineWidth',1.5); % hit - miss
set(gca,'XLim',w_onset); % first triplet (with 25 ms delay);
set(gca,'XTick',0:225:w_onset(2));
...
% plot(t_target,diff(chMUA_target(:,:,1),1,2),'LineWidth',1.5); % ldF - sdF 
plot(t_target,-diff(chMUA_target,1,3),'LineWidth',1.5); % hit - miss
set(gca,'XLim',w_target);
set(gca,'XTick',-225:225:w_target(2));
...
box off;
if strcmp(isLegend,'y')
    legend({'','Small \DeltaF','Large \DeltaF'},'Location',[0.18 0.25 0.1 0.1],'AutoUpdate','off');
end
