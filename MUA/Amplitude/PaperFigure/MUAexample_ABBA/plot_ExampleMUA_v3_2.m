function [] = plot_ExampleMUA_v3_2(ANIMAL,RecordingDate,ch,isLegend)
...
if strcmp(isLegend,'y')
    legend({'Small \DeltaF','Large \DeltaF'},'Location',[0.18 0.85 0.1 0.1],'AutoUpdate','off');
end
...
if strcmp(isLegend,'y')
    legend({'Hit','Miss'},'Location',[0.18 0.55 0.1 0.1],'AutoUpdate','off');
end
...
plot(t_onset,-diff(chMUAb_onset,1,2),'--','LineWidth',1.5); % hit - miss
set(gca,'XLim',w_onset); % first triplet (with 25 ms delay);
set(gca,'XTick',0:225:w_onset(2));
...
plot(t_target,-diff(chMUAb_target,1,2),'LineWidth',1.5); % hit - miss
set(gca,'XLim',w_target);
set(gca,'XTick',-225:225:w_target(2));
...
box off;
if strcmp(isLegend,'y')
    legend({'','Large \DeltaF - Small \DeltaF','Hit - Miss'},'Location',[0.18 0.25 0.1 0.1],'AutoUpdate','off');
end
