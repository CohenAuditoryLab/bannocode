function [SMI,BMI] = getIndex_area_target_ver2(ABB)
...
DATA_DIR = 'C:\Users\Taku\Documents\01_Research\01_STREAM_INTEGRATION&SEGREGATION\ANALYSIS\MUA\code\Amplitude\PaperFigure\IndexComparison_Area\ttime';


% ABB = 'A'; % either A, B1 or B2
% layer = 'I'; % choose depth, either 'S' or 'G' or 'I'

load(fullfile(DATA_DIR,'MUAresponse_area_ttime'));
rCore = rCORE.(ABB);
rBelt = rBELT.(ABB);

% calculate BMI
BMI_core = squeeze( rCore(:,:,1,:) - rCore(:,:,2,:) );
BMI_belt = squeeze( rBelt(:,:,1,:) - rBelt(:,:,2,:) );
% SMI cannot be defiend for ttime because the stdiff was corrupsed in the
% data...
