function [ p_friedman ] = stats_CompLayers_emergenceOnset_Friedman( d_sup, d_deep )
...
% if index dynamically changes along the time
if p_tpos < alpha
    % perform Dunn's test for each area/layer
    i = size(s_string,1);
    X0 = X(1:i,:);
    X1 = X(i+1:end,:);
    Tpos0 = Tpos(1:i,:);
    ...
    disp(['multiple comparison was not performed']);
end
