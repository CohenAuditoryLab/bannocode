...
figure;
subplot(2,2,1);
smi_a = display_Index_LayerArea(SMI_core.A,SMI_belt.A,'Stim');
[p_stim(:,1)]  = stats_CompLayerArea_Friedman(smi_a); % Friedman test
title('A');
subplot(2,2,3);
smi_b1 = display_Index_LayerArea(SMI_core.B1,SMI_belt.B1,'Stim');
[p_stim(:,2)]  = stats_CompLayerArea_Friedman(smi_b1); % Friedman test
title('B1');
subplot(2,2,4);
smi_b2 = display_Index_LayerArea(SMI_core.B2,SMI_belt.B2,'Stim');
...
figure;
subplot(2,2,1);
bmi_a = display_Index_LayerArea(BMI_core.A,BMI_belt.A,'Behav');
[p_behav(:,1)]  = stats_CompLayerArea_Friedman(bmi_a); % Friedman test
title('A');
subplot(2,2,3);
bmi_b1 = display_Index_LayerArea(BMI_core.B1,BMI_belt.B1,'Behav');
[p_behav(:,2)]  = stats_CompLayerArea_Friedman(bmi_b1); % Friedman test
title('B1');
subplot(2,2,4);
bmi_b2 = display_Index_LayerArea(BMI_core.B2,BMI_belt.B2,'Behav');
