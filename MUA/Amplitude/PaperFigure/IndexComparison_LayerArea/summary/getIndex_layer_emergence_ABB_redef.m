function [SMI_layer,BMI_layer] = getIndex_layer_emergence_ABB_redef(ABB,layer)
% 12/17/21 redefine layer assignement 
...
for ff=1:numel(list_RecDate)
    ...
    fName = strcat(RecDate,'_MUAProperty2_ABB');
    load(fullfile(data_dir,fName));
    clear ch_L3 ch_L3u ch_L4 ch_L5
    ...
    elseif strcmp(layer,'I') % infragranular layer
        if ~isnan(ch_L4)
            i_depth(ch_L4+1:end) = 1;
        end
    elseif strcmp(layer,'G') % granular layer
        if ~isnan(ch_L3*ch_L4)
            i_depth(ch_L3:ch_L4) = 1;
        end
    end
    sig_ch(i_depth==0) = [];
