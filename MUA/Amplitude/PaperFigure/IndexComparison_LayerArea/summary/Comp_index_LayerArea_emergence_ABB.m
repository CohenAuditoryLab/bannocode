% get index summary for stats...

clear all;
addpath('C:\Users\Taku\Documents\01_Research\01_STREAM_INTEGRATION&SEGREGATION\ANALYSIS\MUA\code\Amplitude\PaperFigure\IndexComparison_LayerArea\ver2');

load SMIBMI_LayerArea_ABB.mat
% list_SMIBMI = {'SMI','BMI'}; % choose SMI or BMI
SorB = 'SMI';
wTpos = 4:7; % emergence [T-3 T-2 T-1 T]
% wTpos = [1 2 3 6 7] % [1st 2nd 3rd T-1 T]
...
%         std(index_sb(:))/sqrt(length(index_sb(:))) std(index_gb(:))/sqrt(length(index_gb(:))) std(index_ib(:))/sqrt(length(index_ib(:)))];
    ...
    p = stats_CompLayerArea_Friedman(index);
    p_EachArea = stats_CompLayer_EachArea_Friedman(index);
