...
% Combine index with variables...
% core
smi_abb_core = transpose(smi_core);
bmi_abb_core = transpose(bmi_core);
X_core = [smi_abb_core(:) bmi_abb_core(:) area_core(:) tpos_core(:) unit_core(:) ...
    electrode_core(:) layer_core(:) animal_core(:)];
% belt
smi_abb_belt = transpose(smi_belt);
bmi_abb_belt = transpose(bmi_belt);
