function [ p, pp_core, pp_belt, pp_all ] = stats_CompLayerBargraph_KWtest( index )
...
Layer_all = [Layer_core; Layer_belt+3]; % layer index shouldn't overlap...
...
[p_all,tbl_all,stats_all] = kruskalwallis(X_all,Layer_all,'off');
...
% all
if p_all < alpha
    pp_all = multcompare(stats_all,'CType','dunn-sidak','Display','off')
    pp_all = pp_all(:,end);
else
    disp('Becasue p_layer > 0.05, Dunn test has not been performed in layer/area...');
    pp_all = NaN(15,1);
end