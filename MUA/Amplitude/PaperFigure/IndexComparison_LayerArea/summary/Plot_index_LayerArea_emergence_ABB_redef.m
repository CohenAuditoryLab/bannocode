...
for i=1:length(ABB) % index for ABB triplet
    for j=1:length(Layer) % index for layer
        ...
        [smi,bmi] = getIndex_layer_emergence_ABB_redef(ABB{i},Layer{j});
...
if isSave==1
    ...
    save_file_name = 'SMIBMI_LayerArea_ABB_redef';
    save(save_file_name,'SMI_LayerArea','BMI_LayerArea','sTriplet');
end