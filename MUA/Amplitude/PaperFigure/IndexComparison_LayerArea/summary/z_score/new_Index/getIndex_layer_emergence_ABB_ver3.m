function [FMI_layer,SMI_layer,BMI_layer] = getIndex_layer_emergence_ABB_ver3(layer)
...
for ff=1:numel(list_RecDate)
    ...
    fName = strcat(RecDate,'_MUAProperty3_ABB');
    load(fullfile(data_dir,fName));
    
    % depth selection
    i_depth = zeros(length(sig_ch),1);
    if strcmp(layer,'S') % supragranular layer
        if ~isnan(ch_L3) % ch_L3 means bottom of layer 3
            i_depth(1:ch_L3-1) = 1;
        end
