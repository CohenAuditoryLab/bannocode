...
% load zFMISMIBMI_LayerArea_ABB_ver3_3.mat
list_SMIBMI = {'FMI','SMI','BMI'}; % choose SMI or BMI
% list_SMIBMI = {'FMI'}; % just for getting stats...
...
for j=1:numel(list_SMIBMI)
    ...
    if strcmp(SorB,'FMI')
        ...
        % get Index value
        IDX_sCore = zFMI_LayerArea{1,1}; % supragranular layer
        IDX_gCore = zFMI_LayerArea{1,2}; % granular layer
        IDX_iCore = zFMI_LayerArea{1,3}; % infragranular layer
        IDX_sBelt = zFMI_LayerArea{2,1}; % supragranula layer
        IDX_gBelt = zFMI_LayerArea{2,2}; % granular layer
        IDX_iBelt = zFMI_LayerArea{2,3}; % infragranular layer
        x_label = 'zFMI';
