...
for i=1 %1:length(ABB) % index for ABB triplet
    ...
    for j=1:length(Layer)
        ...
        z_fmi_core = zFMI_core(1:nn_core,:);
        z_fmi_belt = zFMI_belt(1:nn_belt,:);
        ...
        % area x layer x ABB
        zFMI_LayerArea{1,j,i} = z_fmi_core;
        zFMI_LayerArea{2,j,i} = z_fmi_belt;
        ...
        % mean of the index...
        mFMI_core = mean(z_fmi_core,1);
        mFMI_belt = mean(z_fmi_belt,1);
        ...
        eFMI_core = std(z_fmi_core,[],1) / sqrt(size(z_fmi_core,1));
        eFMI_belt = std(z_fmi_belt,[],1) / sqrt(size(z_fmi_belt,1));
...
if isSave==1
    ...
    if version==1
        save_file_name = 'zFMISMIBMI_LayerArea_ABB_ver3';
    else
        save_file_name = strcat('zFMISMIBMI_LayerArea_ABB_ver3_',num2str(version));
    end
    save(save_file_name,'zFMI_LayerArea','zSMI_LayerArea','zBMI_LayerArea','sTriplet_fmi','sTriplet_smi','sTriplet_bmi');
end