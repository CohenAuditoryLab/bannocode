...
%         [abb,trp] = get_MUAResponse_AB(t,MUA_stdiff);
            abb = get_zMUAResponse_AB(t,MUA_condition);
            abb_tpos = abb(:,i_tpos,:);
...
%         h = bar(squeeze(mean(zABB_stdiff(j,:,:,i,:),2))); hold on;
%         set(h(1),'FaceColor','r');
%         set(h(2),'FaceColor','b');
...
save(fullfile(SAVE_DIR,params.RecordingDate,'RESP',save_file_name), ...
    'zMUA_A','zMUA_B1','zMUA_B2','zMUA','t','list_st','list_tt');
