%This program computes the area under the MUA for ABB triplet periods
...
%         [abb,trp] = get_MUAResponse_AB(t,MUA_stdiff);
            abb = get_zMUAResponse_ABB(t,MUA_condition,params);
%             abb_tpos = abb(:,i_tpos,:);
%             zABB(:,:,:,i,j,HorM)  = abb_tpos; % ABB x triplet pos x ch x stdiff x ttime x behav
            abb_tpos = abb(i_tpos,:);
            zABB(:,:,i,j,HorM)  = abb_tpos; % triplet pos x ch x stdiff x ttime x behav
%         triplet(:,:,i,HorM) = trp;
        end
    end
end
...
%                 zABB(:,:,:,i,j,k) = zeros(3,length(i_tpos),nChannel);
                zABB(:,:,i,j,k) = zeros(length(i_tpos),nChannel);
            end
...
zABB_stdiff = squeeze(sum(zABB_stdiff,4)); % average across target time
zABB_ttime = squeeze(sum(zABB_ttime,3)); % average across stdiff
...
zABB_stdiff = permute(zABB_stdiff,[1 3 4 2]); % tpos x stdiff x behav x channel
zABB_ttime = permute(zABB_ttime,[1 3 4 2]); % tpos x ttime x behav x channel
zMUA_ABB.stdiff  = zABB_stdiff; % triplet pos x stdiff x behav x channel
zMUA_ABB.ttime  = zABB_ttime; % triplet pos x ttime x behav x channel

zMUA.stdiff = MUA_stdiff;
zMUA.ttime = MUA_ttime;

% save data
save_file_name = strcat(params.RecordingDate,'_zMUAtriplet_ABB');
% save(fullfile(SAVE_DIR,params.RecordingDate,'RESP',save_file_name), ...
%     'zMUA_ABB','zMUA','t','list_st','list_tt');
save(fullfile(SAVE_DIR,params.RecordingDate,'RESP',save_file_name), ...
    'zMUA_ABB','sTriplet','list_st','list_tt');
