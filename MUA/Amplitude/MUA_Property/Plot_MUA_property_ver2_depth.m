...
depth = 'D'; % choose depth, either 'S' or 'G' or 'D'
...
for ff=1:numel(list_RecDate)
    ...
    % depth selection
    i_depth = zeros(size(nPeak_all,2),1);
    if strcmp(depth,'S') % supragranular layer
        if ~isnan(ch_L3)
            i_depth(1:ch_L3-1) = 1;
        end
    elseif strcmp(depth,'D') % infragranular layer
        if ~isnan(ch_L5)
            i_depth(ch_L5:end) = 1;
        end
    elseif strcmp(depth,'G') % granular layer
    ...
    SMI_sig = SMI.(triplet_name); % choose triplet
    SMI_sig(i_depth==0,:) = [];
    SMI_sig(sig_ch==0,:) = [];

    BMI_sig = BMI.(triplet_name); % choose triplet
    BMI_sig(i_depth==0,:) = [];
    BMI_sig(sig_ch==0,:) = [];
...
for tpos=1:n_tpos
    ...
    title(list_tpos{tpos});
    grid on;
    [r_core(tpos),p_core(tpos)] = corr(logPL_core(:,i_selectPL),logSMI_core(:,tpos));
    [r_belt(tpos),p_belt(tpos)] = corr(logPL_belt(:,i_selectPL),logSMI_belt(:,tpos));
end
subplot(3,2,5);
