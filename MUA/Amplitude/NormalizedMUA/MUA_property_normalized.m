...
% RecDate = '20180709'; % just for test the code...

for ff=1:numel(list_RecDate)
    RecDate = list_RecDate{ff};
    data_dir = fullfile(ROOT_DIR,RecDate,'Resp');
    
    fName1 = strcat(RecDate,'_nMUAtriplet');
    fName2 = strcat(RecDate,'_SignificantChannels');
    
    load(fullfile(data_dir,fName1));
    load(fullfile(data_dir,fName2));
    
    smi_a  = get_index_normalize(nMUA_A.stdiff,'SMI');
    fmi_b1 = get_index_normalize(nMUA_B1.stdiff,'FMI');
    fmi_b2 = get_index_normalize(nMUA_B2.stdiff,'FMI');
    bmi_a  = get_index_normalize(nMUA_A.stdiff,'BMI');
    bmi_b1 = get_index_normalize(nMUA_B1.stdiff,'BMI');
    bmi_b2 = get_index_normalize(nMUA_B2.stdiff,'BMI');
...
%     BMI.ABB = bmi_abb';
    ...
    save_file_name = strcat(RecDate,'_MUAProperty_normalize');
