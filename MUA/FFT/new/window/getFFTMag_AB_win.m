function [ nFFTPeak_ST ] = getFFTMag_AB_win( MUA_ST, t, params )
...
t_win = 450; % evaluate A rate (4.4 Hz)
% t_win = 150; % evaluate ABB rate (13.3 Hz)
% t_start = 0:t_win/2:(1500-t_win);
t_start = 0:75:(1500-t_win);
n_window = round( pointspermsec * t_win );
N = length(t_start);

for i=1:N
t_index = find(New_t>=t_start(i),1);
win_index = t_index:(t_index+n_window-1);
mua_window = New_MUA(win_index,:);
mua_hann = hanning(n_window) .* mua_window;
y = fft(mua_hann,[],1);
mag = abs(y) * 2 / n_window; % gets the magnitude
mag = mag(1:(n_window/2),:); % eliminates the negative frequency values
M(:,:,i) = mag;
end
freq = linspace(0,Nyquist,n_window/2);
freq = freq';
...
Y_han = fft(hanning(length(New_MUA)).*New_MUA, 50000,1);
