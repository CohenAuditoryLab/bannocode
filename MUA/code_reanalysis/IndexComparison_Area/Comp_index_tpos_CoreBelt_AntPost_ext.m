...
% load data
% load MUAResponse_area_v1; % version 1: 1st to T triplet
load MUAResponse_area; % extended to T+1 triplet
...
dBehav = display_logIndex_figure_ext(rCORE,rBELT,'Behav'); 
dStim  = display_logIndex_figure_ext(rCORE,rBELT,'Stim');
subplot(2,3,1);
ylabel('log(CI)');
% plot comparing anterior vs posterior
figure("Position",[150 150 800 450]);
ddBehav = display_logIndex_figure_ext(rPOS,rANT,'Behav');
ddStim  = display_logIndex_figure_ext(rPOS,rANT,'Stim'); 
subplot(2,3,1);
ylabel('log(CI)');
