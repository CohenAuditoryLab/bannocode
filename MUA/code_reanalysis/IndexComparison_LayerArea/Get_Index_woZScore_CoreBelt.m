...
% ver 3: 1st (1), T-1 (6), T (7), or T+1 (8) triplet position for all index
tp_fmi = [8]; % triplet position
tp_smi = [8]; % triplet position
tp_bmi = [8]; % triplet position
...
if isSave==1
    ...
    if version==1
        save_file_name = 'FMISMIBMI_LayerArea_ABB_CoreBelt';
    else
        save_file_name = strcat('FMISMIBMI_LayerArea_ABB_CoreBelt_',num2str(version));
    end
