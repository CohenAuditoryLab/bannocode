% check ROC values coming from perfcurv.m function by comparing the output
% from ther other runction (roc_curve.m)
clear all
...
% check ROC values
% version 1: use perfcurve.m
ver = 1;
[AUC_v1,curve_v1] = get_ROC_check(rA_post.sup,rB_post.sup,ver);
% version 2: use roc_curve.m
ver = 2;
[AUC_v2,curve_v2] = get_ROC_check(rA_post.sup,rB_post.sup,ver);

nTpos = length(sTriplet);
figure('Position',[215 560,1160,420]);
for i=1:nTpos
    subplot(2,nTpos,i);
    plot(curve_v1.easy(:,1,i),curve_v1.easy(:,2,i),'LineWidth',1.5); hold on
    plot([0 1],[0 1],':k');
    text(0.1,0.9,num2str(AUC_v1.easy(i)));
    title(sTriplet{i});
    grid on
    subplot(2,nTpos,i+nTpos);
    plot(curve_v2.easy(:,1,i),curve_v2.easy(:,2,i),'LineWidth',1.5); hold on
    plot([0 1],[0 1],':k');
    text(0.1,0.9,num2str(AUC_v1.easy(i)));
    grid on
end

% AUC_post.sup = get_ROC_check(rA_post.sup,rB_post.sup);
% AUC_post.mid = get_ROC_check(rA_post.mid,rB_post.mid);
% AUC_post.dep = get_ROC_check(rA_post.dep,rB_post.dep);
% AUC_ant.sup  = get_ROC_check(rA_ant.sup,rB_ant.sup);
% AUC_ant.mid  = get_ROC_check(rA_ant.mid,rB_ant.mid);
% AUC_ant.dep  = get_ROC_check(rA_ant.dep,rB_ant.dep);
% 
% AUC_all.sup  = get_ROC_check(rA_all.sup,rB_all.sup);
% AUC_all.mid  = get_ROC_check(rA_all.mid,rB_all.mid);
% AUC_all.dep  = get_ROC_check(rA_all.dep,rB_all.dep);
