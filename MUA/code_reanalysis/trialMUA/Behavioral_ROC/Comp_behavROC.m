...
% set parameters...
tpos = 7; % Target triplet


% % % BF site % % %
% load Data
load(fullfile(DATA_DIR,'ROC_Both_ABB_lowBF.mat'));
% get behavAUC at Target...
AUC_post_low = squeeze(AUC.post.behav(:,:,tpos,:,1));
AUC_ant_low  = squeeze(AUC.ant.behav(:,:,tpos,:,1));
clear AUC
...
% % % non-BF site % % %
% load Data
load(fullfile(DATA_DIR,'ROC_Both_ABB_highBF.mat'));
% get behavAUC at Target...
AUC_post_high = squeeze(AUC.post.behav(:,:,tpos,:,1));
AUC_ant_high  = squeeze(AUC.ant.behav(:,:,tpos,:,1));
clear AUC
...
p_post(1) = kruskalwallis(AUC_post_low,[],'off');
p_post(2) = kruskalwallis(AUC_post_high,[],'off');
p_ant(1)  = kruskalwallis(AUC_ant_low,[],'off');
p_ant(2)  = kruskalwallis(AUC_ant_high,[],'off');
