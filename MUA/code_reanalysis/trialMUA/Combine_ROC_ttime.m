...
for ff=1:length(list_RecDate)
    ...
    fName = strcat(rec_date,'_ROC_ttime');
    ...
    if i_AP==1 % posterior
%         if size(auc_ttime,3)==3
%             temp_auc = NaN(n_ch,8,4);
%             temp_auc(:,:,1) = auc_ttime(:,:,1);
%             temp_auc(:,:,4) = auc_ttime(:,:,end);
%             AUC_post_df(1:n_ch,:,:,count_pos) = temp_auc;
%             clear temp_auc
%         else
%             AUC_post_df(1:n_ch,:,:,count_pos) = auc_ttime;
%         end
...
%             temp_auc(:,:,1) = auc_ttime(:,:,1);
%             temp_auc(:,:,4) = auc_ttime(:,:,end);
%             ACU_ant_df(1:n_ch,:,:,count_ant) = temp_auc;
%             clear temp_auc
%         else
%             AUC_ant_df(1:n_ch,:,:,count_ant) = auc_ttime;
%         end
...
% save
save_file_name = strcat('ROC_',Animal_Name,'_ttime');
save_dir = DATA_DIR;
