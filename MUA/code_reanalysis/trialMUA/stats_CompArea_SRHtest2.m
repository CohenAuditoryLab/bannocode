function [ p, tbl, stats_multcomp ] = stats_CompArea_SRHtest2( AUC_abb_a1, AUC_abb_a2, tpos )
...
AUC_abb_a1 = AUC_abb_a1(:,tpos); % data from area1
AUC_abb_a2 = AUC_abb_a2(:,tpos); % data from area2
% list_index = unique(iDepth);
list_index = [0 1 2]; % depth ID (should be 0, 1, or 2)

% remove NaN from data matrix...
auc_area1 = removeNaN_fromMat(AUC_abb_a1);
auc_area2 = removeNaN_fromMat(AUC_abb_a2);

n_tpos  = length(tpos);
n_data1 = size(auc_area1,1);
n_data2 = size(auc_area2,1);
c_string1 = zeros(n_data1,n_tpos);
c_string2 = ones(n_data2,n_tpos);

X = cat(1,auc_area1,auc_area2);
Area = cat(1,c_string1,c_string2);
...
if p_x < alpha
    iArea = Area(:,1);
    X_a1 = X(iArea==0,:); % data from area1
    X_a2 = X(iArea==1,:); % data from area1
...
%         data_dep = X_dep(:,i);
        ...
        % comp area1 vs area2
        [pp,~,stats] = ranksum(data_a1,data_a2);
        pval(1,i) = pp; 
        zval(1,i) = stats.zval;
