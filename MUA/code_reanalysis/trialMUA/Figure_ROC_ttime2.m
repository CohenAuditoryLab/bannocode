...
% load Data
load(fullfile(DATA_DIR,'ROC_Both_ttime2.mat'));
...
AUC_post_ttdf = reshape(AUC.post.ttdf,[size(AUC.post.ttdf,1)*size(AUC.post.ttdf,2) size(AUC.post.ttdf,3) size(AUC.post.ttdf,4)]);
AUC_ant_ttdf  = reshape(AUC.ant.ttdf,[size(AUC.ant.ttdf,1)*size(AUC.ant.ttdf,2) size(AUC.ant.ttdf,3) size(AUC.ant.ttdf,4)]);
...
y_range = [0.4700 0.7000]; % set y-axis 

subplot(2,2,1); hold on;
for ii=1:4
    plot_ROC(tpos,AUC_post_ttdf(:,:,ii),jitter(ii),line_color(ii,:));
end
...
for ii=1:4
    plot_ROC(tpos,AUC_ant_ttdf(:,:,ii),jitter(ii),line_color(ii,:));
end
set(gca,'XTick',tpos,'XTickLabel',{'early','','','late'});
xlim([tpos(1)-0.5 tpos(end)+0.5]);
ylim(y_range);
