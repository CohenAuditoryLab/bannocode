% modify lines 50-53, lines 70-75 of Extract_FreqTuning.m
% get selectivity and sparseness index
clear all
...
for ff=1:numel(list_RecDate)
    ...
    % use mean A resp in largest dF conditions...
    A_ldf = A_stdiff(:,:,end);
    % obtain selectivity/sparseness index
    % selectivity index version 1
    [sl_index,sp_index] = calc_SLSPindex(A_ldf,B1_stdiff,1);
    % selectivity index version 2
    [sl_index2,sp_index] = calc_SLSPindex(A_ldf,B1_stdiff,2);
    ...
    % obtain selectivity/sparseness index
    % use mean A resp in largest dF conditions...
    A_ldf_h = A_std_h(:,:,end); A_ldf_m = A_std_m(:,:,end);
    % version 1
    [sl_index_h,sp_index_h] = calc_SLSPindex(A_ldf_h,B1_std_h,1);
    [sl_index_m,sp_index_m] = calc_SLSPindex(A_ldf_m,B1_std_m,1);
    % version 2
    [sl_index2_h,sp_index_h] = calc_SLSPindex(A_ldf_h,B1_std_h,2);
    [sl_index2_m,sp_index_m] = calc_SLSPindex(A_ldf_m,B1_std_m,2);
    ...
    % save
    save_file_name = strcat(RecDate,'_FreqTuning2');
    save(fullfile(SAVE_DIR,'ver2',save_file_name),'select_index','select_index2','sparse_index', ...
