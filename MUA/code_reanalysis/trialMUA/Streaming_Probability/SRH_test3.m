function [p, Table] = SRH_test3( Data, var1_name, var2_name, var3_name )
% Scheirer-Ray-Hare test 
% modified by TB to output p-value
% original code downloaded from Matlab central 
% https://www.mathworks.com/matlabcentral/fileexchange/96399-non-parametric-alternative-of-2-way-anova-scheirerrayhare
% further modified from SRH_test.m to perform 3-way ANOVA without examining
% interactions
...
%     T = table(Data(:,1),Data(:,2),Data(:,3),'VariableNames',{'measure',var1_name,var2_name});
    T = table(Data(:,1),Data(:,2),Data(:,3),Data(:,4),'VariableNames',{'measure',var1_name,var2_name,var3_name});
    
    % Construct linear model
%     M = fitlm(T,['measure ~ ',var1_name,' + ',var2_name,' + ',var1_name,':',var2_name],...
%               'CategoricalVars',{var1_name,var2_name});
    M = fitlm(T,['measure ~ ',var1_name,' + ',var2_name,' + ',var3_name],...
              'CategoricalVars',{var1_name,var2_name,var3_name});
...
%     Factors = {var1_name, var2_name, cat(2,var1_name,' x ', var2_name),'Residuals'};
    Factors = {var1_name, var2_name, var3_name,'Residuals'};
    
%     % Construct output table
%     out = table(Factors',MS(:,1),MS(:,2),MS(:,4),MS(:,5),'VariableNames',{'Factor','SS','DF','H','pvalue'});

    % modified by TB
    Table = table(Factors',MS(:,1),MS(:,2),MS(:,4),MS(:,5),'VariableNames',{'Factor','SS','DF','H','pvalue'});
    p = MS(:,5);
end