...
% % CHOOSE BF_SITES FOR THIS ANALYSIS!!!
...
% pStream_post_hm2 = NaN(24,8,2,n_pos); % small and large dF trials
% pStream_ant_hm2 = NaN(24,8,2,n_ant); % small and large dF trials
...
%             AUC_ant_df(1:n_ch,:,:,count_ant) = temp_auc;
            ...
            temp_ps1(:,:,1) = p_1srm(:,:,1);
            temp_ps1(:,:,4) = p_1srm(:,:,end);
            pStream_ant_df1(1:n_ch,:,:,count_pos) = temp_ps1;

            temp_ps2 = NaN(n_ch,8,4);
            temp_ps2(:,:,1) = p_2srm(:,:,1);
            temp_ps2(:,:,4) = p_2srm(:,:,end);
            pStream_ant_df2(1:n_ch,:,:,count_pos) = temp_ps2;
...
% save
save_dir = DATA_DIR;
save_file_name = strcat('StreamingProb_',Animal_Name,'_',BF_Session,'BF');
if strcmp(behavOut,'hit')
    save_file_name = strcat(save_file_name,'_hit');
elseif strcmp(behavOut,'miss')
    save_file_name = strcat(save_file_name,'_miss');
end
save(fullfile(save_dir,save_file_name),'pStream','list_RecDate','sTriplet', ...
