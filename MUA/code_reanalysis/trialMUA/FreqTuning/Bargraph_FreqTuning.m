...
% mean
mIDX_post = [nanmean(IDX_post_low,1); nanmean(IDX_post_high,1)];
mIDX_ant = [nanmean(IDX_ant_low,1); nanmean(IDX_ant_high,1)];

% median
mdIDX_post = [nanmedian(IDX_post_low,1); nanmedian(IDX_post_high,1)];
mdIDX_ant = [nanmedian(IDX_ant_low,1); nanmedian(IDX_ant_high,1)];

% standard error
n_post = [sum(~isnan(IDX_post_low),1); sum(~isnan(IDX_post_high),1)];
n_ant  = [sum(~isnan(IDX_ant_low)); sum(~isnan(IDX_ant_high),1)];
s_post = [nanstd(IDX_post_low,[],1); nanstd(IDX_post_high,[],1)]; % standard deviation
s_ant  = [nanstd(IDX_ant_low,[],1); nanstd(IDX_ant_high,[],1)];
...
% h_bf(1)  = plot(X([1 3]),mIDX_post(:,1),':+'); % plot mean
% h_nbf(1) = plot(X([2 4]),mIDX_post(:,2),':+');
% plot(X([1 3]),mIDX_post(:,1),'x','MarkerSize',10); % plot mean
% plot(X([2 4]),mIDX_post(:,2),'x','MarkerSize',10);
...
% h_bf(2)  = plot(X([1 3]),mIDX_ant(:,1),'+'); % mean
% h_nbf(2) = plot(X([2 4]),mIDX_ant(:,2),'+');
