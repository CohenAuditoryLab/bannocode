...
for i=1:length(ABB) % index for ABB triplet
    for j=1:length(Layer) % index for layer
        ...
        [smi,bmi] = getIndex_reDefLayer_antpost(ABB{i},Layer{j},tpos);
...
elseif numel(Layer)==3
    legend({'poserior superficial','anterior superficial','posterior granular','anterior granular','posterior deep','anterior deep'},'Location',[0.5 0.8 0.2 0.1]);
end