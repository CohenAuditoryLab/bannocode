...
tROC_BF_post = squeeze(AUC.post.deviance(:,:,:,ii));
tROC_BF_ant  = squeeze(AUC.ant.deviance(:,:,:,ii));
...
% obtain ROC for target (ch x session x stdiff x all-hit-miss)
tROC_nonBF_post = squeeze(AUC.post.deviance(:,:,:,ii));
tROC_nonBF_ant  = squeeze(AUC.ant.deviance(:,:,:,ii));
y_range = [0.1 0.9];
% if strcmp(plot_type,'dev')
%     % devidation from 0.5
