...
% plot the change in sROC (1st vs T-1) against d-prime
...
tpos = [1 6]; % choose 1st and (T-1) triplet positions
% tpos = [6 7]; % choose (T-1) and (T) triplet positions
...
% difference between 1st and T-1 triplet (1st - T-1)
dROC_BF_post = -1 * diff(tROC_BF_post,1,3);
dROC_BF_ant  = -1 * diff(tROC_BF_ant,1,3);
dROC_BF_post = squeeze(dROC_BF_post);
dROC_BF_ant = squeeze(dROC_BF_ant);
...
% % difference between 1st and T-1 triplet (T-1 - 1st)
% dROC_nonBF_post = -1 * diff(tROC_nonBF_post,1,3);
% dROC_nonBF_ant  = -1 * diff(tROC_nonBF_ant,1,3);
% difference between 1st and T-1 triplet (T-1 - 1st)
dROC_nonBF_post = 1 * diff(tROC_nonBF_post,1,3);
dROC_nonBF_ant  = 1 * diff(tROC_nonBF_ant,1,3);
dROC_nonBF_post = squeeze(dROC_nonBF_post);
dROC_nonBF_ant = squeeze(dROC_nonBF_ant);
...
% scatter plot
mtROC_BF_post = squeeze(nanmean(dROC_BF_post,1)); % mean across channels
mtROC_BF_ant = squeeze(nanmean(dROC_BF_ant,1));
mtROC_nonBF_post  = squeeze(nanmean(dROC_nonBF_post,1));
mtROC_nonBF_ant  = squeeze(nanmean(dROC_nonBF_ant,1));
...
xlabel('d prime'); ylabel('delta sROC');
