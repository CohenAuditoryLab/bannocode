function [SMI,BMI] = get_SMIBMI_channel_v2(rALL_ABB,area_index,AP_index,toneID)
...
[smi_c,bmi_c] = get_Index_channel(r_core,toneID); % core
[smi_b,bmi_b] = get_Index_channel(r_belt,toneID); % belt
[smi_p,bmi_p] = get_Index_channel(r_post,toneID); % posterior
[smi_a,bmi_a] = get_Index_channel(r_ant,toneID);  % anterior
...
function [smi,bmi] = get_Index_channel(rALL,toneID)
...
for i_tpos = 1:n_tpos
    ...
    [smi(:,i_tpos),bmi(:,i_tpos)] = calc_Index_channel(Rmat,toneID);
end
...
% % calc_Index_channel
function [smi,bmi] = calc_Index_channel(Rmatrix,toneID)
Rhh = Rmatrix(:,1);
Reh = Rmatrix(:,2);
Rhm = Rmatrix(:,3);
Rem = Rmatrix(:,4);
