...
addpath(genpath('C:\work\KiloSort')) % path to kilosort folder
addpath(genpath('C:\work\phy')) % path to npy-matlab scripts
pathToYourConfigFile = 'C:\work\KiloSort';
% specify location to store data files for this simulation
% run the config file to build the structure of options (ops)
run(fullfile(pathToYourConfigFile, 'config_original_domo.m'));
if ops.GPU, gpuDevice(1); end % initialize GPU
%Where files live
folders = dir('Z:\Taku\kilosort_Domo\ABBA');
