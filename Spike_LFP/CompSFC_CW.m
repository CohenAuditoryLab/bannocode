...
for ch=1:n_ch
    ...
    % display figure
    [coh_c,cgm_c,H(1)] = calc_SFC(lfp_c,raster_c,t_raster/1000,aInfo);
    [coh_w,cgm_w,H(2)] = calc_SFC(lfp_w,raster_w,t_raster/1000,aInfo);
...
%     title(['channel ' num2str(ch)]);
    ...
    s_dir = fullfile(SAVE_DIR,'SFC_depth',sessionID,alignment,unit,electrodeID_lfp); % save directory
    s_name = strcat('SFC_',electrodeID_spk,'_#',num2str(cl),'_', ...
        electrodeID_lfp,'_ch',num2str(ch),'_'); % save file name
    ext = {'correct','wrong','diff'};
    for i=1:3
