...
for ch=1:n_ch
    ...
    % display figure
    [coh,cgm,H(1)] = calc_SFC(lfp_ch,raster_cl,t_raster/1000,aInfo);
    [coh_c,cgm_c,H(2)] = calc_SFC(lfp_c,raster_c,t_raster/1000,aInfo);
    [coh_w,cgm_w,H(3)] = calc_SFC(lfp_w,raster_w,t_raster/1000,aInfo);
    ...
    % plot figure
    H(4) = figure('Position',[50 50 500 700]);
    subplot(3,1,3);
...
%     title(['channel ' num2str(ch)]);
    ...
    s_dir = fullfile(SAVE_DIR,sessionID,alignment,unit,electrodeID_lfp); % save directory
    ...
    for i=1:4
        ...
        saveas(H(i),fullfile(s_dir,ss),'png');
    end
    close all;
...
s_dir = fullfile(SAVE_DIR,sessionID,alignment,unit);
