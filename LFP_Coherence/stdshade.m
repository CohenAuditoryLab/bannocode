...
n_trial = size(amatrix,1);
astd = nanstd(amatrix,[],1); % to get std shading
% aste = astd / sqrt(n_trial);
...
%     fillOut = fill([F fliplr(F)],[amean+aste fliplr(amean-aste)],acolor,'linestyle','none');
    acolor='k';
else
    fillOut = fill([F fliplr(F)],[amean+astd fliplr(amean-astd)],acolor, 'FaceAlpha', alpha,'linestyle','none');
%     fillOut = fill([F fliplr(F)],[amean+aste fliplr(amean-aste)],acolor, 'FaceAlpha', alpha,'linestyle','none');
end
if ishold==0
