function [] = plot_coherogram(t,f,Coherogram,string,ch)
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
n_ch = size(Coherogram,1); % number of channel
% c = parula(n_ch); % specify color map to use
% c = jet(n_ch); % specify color map to use

if n_ch==14
    list_ch = {'ch02','ch03','ch04','ch05','ch06','ch07','ch08','ch09','ch10','ch11','ch12','ch13','ch14','ch15'};
    iCh = zeros(1,14);
elseif n_ch==20
    list_ch = {'ch03','ch04','ch05','ch06','ch07','ch08','ch09','ch10','ch11','ch12','ch13','ch14','ch15','ch16','ch17','ch18','ch19','ch20','ch21','ch22'};
    iCh = zeros(1,20);
end

for i=1:length(ch)
    iCh = iCh + contains(list_ch,num2str(ch(i)));
end
imagesc(t,f,squeeze(mean(Coherogram(iCh==1,:,:),1))); set(gca,'YDir','normal');
xlabel('Time [sec]'); ylabel('Frequency [Hz]');

% if strcmp(isLabel,'y')||strcmp(isLabel,'Y')||isLabel==1
% if n_ch==14 % 16-ch electrode
%     ch_label = {'ch02','ch03','ch04','ch05','ch06','ch07','ch08','ch09','ch10','ch11','ch12','ch13','ch14','ch15'};
% elseif n_ch==20 % 24-ch electrode
%     ch_label = {'ch03','ch04','ch05','ch06','ch07','ch08','ch09','ch10','ch11','ch12','ch13','ch14','ch15','ch16','ch17','ch18','ch19','ch20','ch21','ch22'};
% end
% legend(ch_label);
% end

s = inputname(3);
if length(ch)==1
    string = [string ' ch' num2str(ch)];
else
    string = [string ' ch' num2str(ch(1)) ' to ' num2str(ch(end))];
end

if strcmp(s(end),'c')
    s_title = [string ' correct'];
elseif strcmp(s(end),'w')
    s_title = [string ' wrong'];
elseif strcmp(s(end),'d')
    s_title = [string ' difference'];
end
title(s_title);
colormap jet;

end

