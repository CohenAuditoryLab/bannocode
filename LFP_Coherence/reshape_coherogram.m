function [ coh_new ] = reshape_coherogram( tcoh, iElectrode_AC, iElectrode_PFC )
%reshape_coherence Summary of this function goes here
%   select and reshape coherence calculated by fieldtrip toolbox
%   tcoh: struct of coherogram from fieldtrip
%   iElectrode_AC: AC electrode ID
%   iElectrode_PFC: PFC electrode ID

lb_cmb = tcoh.labelcmb; % channel combination label
C = tcoh.cohspctrm; % original matrix of coherence (cannel_cmb x freq x time)
...
n_tp = size(C_select,3); % number of time point

% reshape matrix
C_new = reshape(C_select,n_a,n_b,n_fp,n_tp);
...
coh_new.cohspctrm_mat = C_new; % in matrix form (AC ch x PFC ch x freq x time)
% remove updated fields (to avoid confusion)
cfg = coh_new.cfg;
cfg = rmfield(cfg,'channelcmb');
cfg = rmfield(cfg,'channel');
coh_new.cfg = cfg; % update
end
