...
%         y = ft_preproc_bandstopfilter(y,Fnew,[59 61]); % 4th order filter
...
%     % zero-padding data
%     [bipolar_lfp_pad,tt_pad] = zeropadLFP(bipolar_lfp_rmNoise,tt,2000);
    
    % put the data into cell
    trial{n} = bipolar_lfp_rmNoise; %transpose(bipolar_lfp_rmNoise);
    time{n}  = tt;
%     trial{n} = bipolar_lfp_pad; %transpose(bipolar_lfp_rmNoise);
%     time{n}  = tt_pad;
end

% insert random number in sample info...
nSample = size(bipolar_lfp_rmNoise,2); % samples in resampled data
% nSample = size(bipolar_lfp_pad,2); % samples in resampled data
s_start = 1;
for j=1:nTrial
