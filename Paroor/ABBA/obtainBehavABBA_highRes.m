...
SAVE_FILE_DIR = 'D:\Cassius\04_ABBA';
% set variables
% Block = '~20200114_ABBA_d02'; % frequency task
% Block = '~20180905_ABBAl_d01'; % location task
% Block = '~202011_ABBA_test';
Block = '~20210410_ABBA_d03';
FileName_LabVIEW = 'cassius_frq_dynamic_2021041001';
DataTimeRange = [-0.5 3.5];
DelayStimOn = -1.5e-3; % actual onset of the stimulus is ~2.5 ms before the estimated time
% ErrorStimOn = 0;

% set global variables (should NOT change)
% TANK_FILE_DIR = 'F:\TDT\Synapse\Tanks\Domo-200114';
TANK_FILE_DIR = 'D:\TDT\Tanks\Cassius-210410';
...
% tranges(:,18) = []; % 12/23/20 TDT assigned empty trials...
...
calib_window = round([tranges(2,1)-6 tranges(2,1)-5]*SF_stim);
...
t_stim = t_stim * 1000;
t_hand = t_hand * 1000;
% stim_threshold = std(stim_noNoise(calib_window(1):calib_window(2))) * 5.0; % should be ~0.0023
stim_threshold = 0.01; %0.0025; % must be larger than check_th_val
% stim_threshold = 0.03; % 3/6/21 for 200Hz
% stim_threshold = 0.0055; % 12/21/20 for 200Hz
...
%     stim_on_time = tranges(2,n); % in sec
    ...
    Stim(n,:) = stim_noNoise(data_onset_stim:data_onset_stim+floor(diff(DataTimeRange)*SF_stim)) - baseline; % stim in trial
...
for i=1:numel(iTE)
    figure;
    plot(t_hand,Rew(iTE(i),:),'Color',[.9 .8 .8]); hold on
    plot(t_hand,Lever2(iTE(i),:),'Color',[.5 .5 .5]);
    plot(t_hand,Lever1(iTE(i),:),'Color',[.5 .5 .5]);
    plot(t_stim,Stim(iTE(i),:) * 4,'k');
%     plot([targetTime(iTE(i)) targetTime(iTE(i))+50],[0 0],'r','LineWidth',3);
    set(gca,'ylim',[-5 10]);
...
disp('saving the behavioral data...');
save_file_name = [date '_ABBA_BehavData.mat'];
save(fullfile(SAVE_FILE_DIR,date,save_file_name),'param','tranges', ...
