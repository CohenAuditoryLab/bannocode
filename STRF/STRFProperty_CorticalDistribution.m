...
for ff=1:length(list_RecDate)
    ...
    if i_area==1 % core
        ...
        plot(median(AP),median(log2(params_sig(:,3)/200)),'bo','LineWidth',2);
%         figure(2); % plot latency
%         plot(median(LM),median(params_sig(:,5)),'o');
%         stem3(AP,LM,log2(params_sig(:,3)),'o');
    elseif i_area==0 % belt
        params_belt = cat(1,params_belt,params_sig);
        figure(1); % plot best frequency
        plot(median(AP),median(log2(params_sig(:,3)/200)),'r^','LineWidth',2);
%         figure(2); % plot latency
%         plot(median(LM),median(params_sig(:,5)),'^');
