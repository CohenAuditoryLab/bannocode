...
Block = '~20180711_Ripple2_d01'; % v-probe
% for ch=8 % single electrode
for ch=1:16 % v-probe
    %     [Data] = readtank_mwa_input_tb('C:\TDT\Synapse\Tanks\Domo-180227-115025','~20180516_Ripple_u03',s,'local');
    %     [Data] = readtank_mwa_input_tb('C:\TDT\Synapse\Tanks\Domo-180227-115025','~20180521_Ripple_u02',s,'local');
...
%          [TrigA,TrigB]=trigfixstrf2(TrigTimes,400,902/2); % single stim presentation
    [TrigA,TrigB]=trigfixstrf2(TrigTimes,400,902); % double stim presentation
    ...
    if ch<64
        ...
        [taxis,faxis,STRF1A,STRF2A,PP,Wo1A,Wo2A,No1A,No2A,SPLN]=rtwstrfdb('D:\STRF_files\ripple_96k\DNR_Cortex_96k5min.spr',0,0.15,spet,TrigA,fs,80,30,'dB','MR',300,'float');
        ...
        [taxis,faxis,STRF1B,STRF2B,PP,Wo1B,Wo2B,No1B,No2B,SPLN]=rtwstrfdb('D:\STRF_files\ripple_96k\DNR_Cortex_96k5min.spr',0,0.15,spet,TrigB,fs,80,30,'dB','MR',300,'float');
...
%         text(100,7,['BF = ' num2str(RF1P(ch).BFHz) ' Hz'],'FontWeight','bold');
        ...
        print(['D:\Taku\03_STRF\' saveFileName],'-djpeg');
        %   contaML=contaML+1;
        close all
        %      if ch<32
