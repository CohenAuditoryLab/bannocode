...
%
%   (C) Monty A. Escabi, Edit October 2007 (Edit March 2009)
%
function [TFParam]=rtfparam(Fm,RD,RTF,Thresh,alpha,Display)
...
% Nt=(length(MTF)-1)/2; %original
% Fmb=Fm(Nt+1:2*Nt+1); %original
% tMTF(1)=MTF(Nt+1); %original
% tMTF(2:Nt+1)=(MTF(Nt:-1:1)+MTF(Nt+2:2*Nt+1))/2; %original
Nt = length(MTF) / 2; %tb
Fmb = Fm(Nt+1:2*Nt); %tb
tMTF(1:Nt) = (MTF(Nt:-1:1)+MTF(Nt+1:2*Nt)) / 2; %tb
tMTF=tMTF;
...
RTF2=RTF;
RTF2(i)=zeros(size(i));
MTF=mean(RTF2);         %Note that RTF is ^2 so we are dealing with Power
%MTF=MTF-min(MTF);      %Remove DC
% Nt=(length(MTF)-1)/2; %original
% Fmb=Fm(Nt+1:2*Nt+1); %original
% tMTF(1)=MTF(Nt+1); %original
...
tMTFn=tMTF;
cTMF2=sum(tMTFn.*Fmb)/sum(tMTFn);
bwTMF2=2*sqrt(sum(tMTFn.*(Fmb-cTMF).^2)/sum(tMTFn));
...
sMTFn=sMTF;
cSMF2=sum(sMTFn.*RDb)/sum(sMTFn);
bwSMF2=2*sqrt(sum(sMTFn.*(RDb-cSMF).^2)/sum(sMTFn));
...
if strcmp(Display,'y')
    ...
    plot(cTMF2*[-1 1],cSMF2*[1 1],'g+','linewidth',5)
    hold on
    plot(cTMF2*[1 1],cSMF2+bwSMF2/2*[1 -1],'g-','linewidth',5)
    plot(cTMF2+[1 -1]*bwTMF2/2,cSMF2*[1 1],'g-','linewidth',5)
