function [ STRFData ] = getSTRF_fromTDT( ANIMAL, DATA_DIR, DATA_FILE_NAME, SPR_DIR, display_fig )
%obtain STRF for statistical significance of STRF
...
nBoot = 5; %8; % get 5 STRFs from 5 min DMR stim
% nBoot = 10; % get 10 STRFs from 5 min DMR stim
...
%     shSpet = shufflespet(spet);
    ...
    [taxis,faxis,STRF1A,STRF2A,PP,Wo1A,Wo2A,No1A,No2A,SPLN]=rtwstrfdbintboot(SPR_DIR,0,0.15,spet,TrigA,fs,80,30,'dB','MR',1700,5,'float',nBoot);
    ...
    [taxis,faxis,STRF1B,STRF2B,PP,Wo1B,Wo2B,No1B,No2B,SPLN]=rtwstrfdbintboot(SPR_DIR,0,0.15,spet,TrigB,fs,80,30,'dB','MR',1700,5,'float',nBoot);
...
% % save data
% disp('saving STRF...');
% save_file_name = [Block(2:9) '_STRF' Block(end-3:end)];
