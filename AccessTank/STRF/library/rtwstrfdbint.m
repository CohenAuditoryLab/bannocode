%
%function [taxis,faxis,STRF1,STRF2,PP,Wo1,Wo2,No1,No2,SPLN]=rtwstrfdbint(SpecFile,
%T1,T2,spet,Trig,Fss,SPL,MdB,ModType,Sound,NBlocks,UF,sprtype)
%
%       FILE NAME       : RT WSTRF DB INT
...
%
function [taxis,faxis,STRF1,STRF2,PP,Wo1,Wo2,No1,No2,SPLN]=rtwstrfdbint(SpecFile,T1,T2,spet,Trig,Fss,SPL,MdB,ModType,Sound,NBlocks,UF,sprtype)
...
if strcmp(ModType,'dB')
	...
	while ~feof(fid) & TrigCount<length(Trig)-1
		...
		for k=1:length(spettrig1)
			...
			if L < N2
				STRF1(:,:,kk1)=STRF1(:,:,kk1) + MdB*[S1(:,M-(N2-L-1):M) S2(:,1:L+N1)] - RMSP;
			elseif L+N1 > M
				STRF1(:,:,kk1)=STRF1(:,:,kk1) + MdB*[S2(:,L-N2+1:M) S3(:,1:N1-M+L)] - RMSP;
			else
				STRF1(:,:,kk1)=STRF1(:,:,kk1) + MdB*[S2(:,L-N2+1:L+N1)] - RMSP;
			end
		...
		for k=1:length(spettrig2)
			...
			%Averaging Pre-Event Spectral Profiles
			if L < N1
				STRF2(:,:,kk2)=STRF2(:,:,kk2) + MdB*[S1(:,M-(N1-L-1):M) S2(:,1:L+N2)] - RMSP;
			elseif L+N2 > M
				STRF2(:,:,kk2)=STRF2(:,:,kk2) + MdB*[S2(:,L-N1+1:M) S3(:,1:N2-M+L)] -RMSP;
			else
				STRF2(:,:,kk2)=STRF2(:,:,kk2) + MdB*[S2(:,L-N1+1:L+N2)] - RMSP;
			end
...
%		MeanP=mean(10.^(MdB*fread(fid,1024*256,'float')/10));   % Same as RMSP but need to compute Numerically 
	...
	while ~feof(fid) & TrigCount<length(Trig)
		...
		for k=1:length(spettrig1)
			...
			%Averaging Pre-Event Spectral Profiles
			if L < N2
				STRF1(:,:,kk1)=STRF1(:,:,kk1)+ ( 10.^(MdB*[S1(:,M-(N2-L-1):M) S2(:,1:L+N1)]/20) - MeanP ) / RMSP*sqrt(PP);
			elseif L+N1 > M
				STRF1(:,:,kk1)=STRF1(:,:,kk1)+ ( 10.^(MdB*[S2(:,L-N2+1:M) S3(:,1:N1-M+L)]/20) - MeanP ) / RMSP*sqrt(PP);
			else
				STRF1(:,:,kk1)=STRF1(:,:,kk1)+ ( 10.^(MdB*S2(:,L-N2+1:L+N1)/20) - MeanP ) / RMSP*sqrt(PP);
			end
		...
		for k=1:length(spettrig2)
			...
			%Averaging Pre-Event Spectral Profiles
			if L < N1
				STRF2(:,:,kk2)=STRF2(:,:,kk2)+ ( 10.^(MdB*[S1(:,M-(N1-L-1):M) S2(:,1:L+N2)]/20) - MeanP ) / RMSP*sqrt(PP);
			elseif L+N2 > M
				STRF2(:,:,kk2)=STRF2(:,:,kk2)+ ( 10.^(MdB*[S2(:,L-N1+1:M) S3(:,1:N2-M+L)]/20) - MeanP ) / RMSP*sqrt(PP);
			else
				STRF2(:,:,kk2)=STRF2(:,:,kk2)+ ( 10.^(MdB*S2(:,L-N1+1:L+N2)/20) - MeanP ) / RMSP*sqrt(PP);
			end
